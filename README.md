...
**Sistemas y redes**  
`Windows Server 2022` `Active Directory` `Samba4` `Ubuntu` `Kali Linux` `Arch Linux`  
`TCP/IP` `DNS` `DHCP` `SSH` `NFS` `FTP`
 
**Seguridad ofensiva**  
`nmap` `Hydra` `Metasploit` `Burp Suite` `BloodHound` `Impacket` `CrackMapExec` `searchsploit`  
`Reconocimiento` `Fuerza bruta` `Enumeración` `Escalada de privilegios` `AD Attacks`
